...
#       Fetch default values for the fields
        ...
        for row in file_data:
            ...
            if not row[0] or not row[2] or not row[3] or not row[5] or not row[6]:
                raise ValidationError(
                    "Please Assign The Product Name,Type,Uom And Category.")
            ...
            cust_tax = Tax.search(
                [('name', '=', float(row[19])), ('type_tax_use', '=', 'sale')], limit=1)
            vend_tax = Tax.search(
                [('name', '=', float(row[20])), ('type_tax_use', '=', 'purchase')], limit=1)
            ...
            if row[13]:
                if partner_id:
                    ...
                    partner_id = Partner.create(
                        {'name': row[13], 'supplier': True, 'customer': False, 'compnay_type': 'company'}).id
                new_pro_up.update(
                    {'seller_ids': [(0, 0, {'min_qty': 1, 'name': partner_id, 'price': row[8]})]})
            ...
            if uom_id:
                ...
                uom_id = Uom.create(
                    {'name': row[5], 'category_id': uom_categ_id.id}).id
            if purch_uom_id:
                purch_uom_id = purch_uom_id.id
            else:
                purch_uom_id = Uom.create(
                    {'name': row[6], 'category_id': uom_categ_id.id}).id
            ...
            if row[19]:
                if cust_tax:
                    ...
                    cust_tax = Tax.create(
                        {'name': float(row[19]), 'type_tax_use': 'sale', 'amount': float(row[19])}).id

            if row[20]:
                if vend_tax:
                    vend_tax = vend_tax.id
                else:
                    vend_tax = Tax.create({'name': float(
                        row[20]), 'type_tax_use': 'purchase', 'amount': float(row[20])}).id
            new_pro_up.update({
                'name': row[0],
                'default_code': row[1],
            ...
            p_id = Product.search([('default_code', '=', str(
                int(row[1])) if isinstance(row[1], float) else row[1])], limit=1)
            if p_id:

...
class Purchase_wizard(models.TransientModel):
    ...
    state_stage = fields.Selection(
        [('draft', 'Draft'), ('purchase', 'Purchase')], string='Import State')
...
#           Search if not found it will create if create option is selected
            ...
            if partner_id:
                ...
                if self.option == 'create':
                    ...
                    Log.create(
                        {'operation': 'po', 'message': 'Skipped could not find the partner with name %s' % str(row[1])})
                    continue
#           Search if not found it will create if create option is selected
            ...
            if currency_id:
                ...
                if self.option == 'create':
                    ...
                    Log.create(
                        {'operation': 'po', 'message': 'Skipped could not find the currency with name %s' % str(row[2])})
                    continue
...
#           Search if not found it will create if create option is selected
            ...
            if product_id:
                ...
                if self.option == 'create':
                    ...
                    Log.create({'operation': 'po', 'message': 'Skipped could not find the product with code %s' % (
...
#           Search if not found it will create if create option is selected
            ...
            if uom_id:
                ...
                if self.option == 'create':
                    ...
                    Log.create(
                        {'operation': 'po', 'message': 'Skipped could not find the uom with name %s' % str(row[7])})
                    continue
#           Search if not found it will create if create option is selected
            cust_tax = Tax.search(
                [('name', '=', float(row[10])), ('type_tax_use', '=', 'purchase')], limit=1)
            if row[10]:
                if cust_tax:
                    cust_tax = cust_tax.id
                else:
                    if self.option == 'create':
                        cust_tax = Tax.create({'name': float(
                            row[10]), 'type_tax_use': 'purchase', 'amount': float(row[10])}).id
                    else:
                        Log.create(
                            {'operation': 'po', 'message': 'Skipped could not find the tax with name %s' % float(row[10])})
                        continue

        ...
        for tf in self:
            if 'active_ids' in self._context:
                ...
                if transfer.state == 'approved':
                    ...
                    types = type_obj.search([('default_location_src_id', '=', transfer.source_warehouse_id.lot_stock_id.id), ('code', '=', 'outgoing')], limit=1)
                    ...
                    for user in transfer.dest_warehouse_id.user_ids:
                        transfer.make_activity_user(user)
                    ...
                    type_obj = self.env['stock.picking.type']
                    types = type_obj.search([('default_location_dest_id', '=', transfer.dest_warehouse_id.lot_stock_id.id), ('code', '=', 'incoming')], limit=1)

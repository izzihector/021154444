        ...
        for grupo, balance in ragrp:
            ...
            if accxsubgrp:
                ...
                if tiene_subgrupo:
                    ...
                    res[gtClave]['name'] = "SUB-TOTAL " + \
                        grp_id.name if grp_id.name else ''
    ...
    def generate_xlsx_report(self, workbook, data, obj):
        ...
        for x in sheet_report:
            ...
            for k in res:
                ...
                sheet.merge_range(
                    'R7:S7', ("Realisatie Vorig Jaar"), format22box_bot)
                sheet.merge_range(
                    'U7:V7', ("Realisatie Vorig Maand"), format22box_bot)
                ...
                if data['form'].get('tag_id', False):
                    ...
                    sheet.write(row_number, col_number + 1, '=SUM(C'+str(row_initial +
                                                                         2) + ':C' + str(row_sum) + ')-'+str(balance_dif), money)
                    sheet.write(row_number, col_number + 4, '=SUM(F' +
                                str(row_initial+2) + ':F' + str(row_sum) + ')', money)
                    sheet.write(row_number, col_number + 7, '=SUM(I'+str(row_initial+2) +
                                ':I' + str(row_sum) + ')-'+str(year_prior_bal_dif), money)
                    sheet.write(row_number, col_number + 10, '=SUM(L'+str(row_initial+2) +
                                ':L' + str(row_sum) + ')-'+str(month_prior_bal_dif), money)
